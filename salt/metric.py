...
class Score:
    ...
    def __call__(self, p_mask, target: torch.Tensor):
        """
...
class EmptyTP:
    ...
    def __call__(self, p_mask, target):
        p_no_salt_index = ~((p_mask>0.5).view(p_mask.shape[0], -1).any(dim=1))
...
class HasSaltScore:
    ...
    def __call__(self, p_mask, target):
        ...
        if t_has_salt_index.any():
            s = get_score(p_mask[t_has_salt_index], target[t_has_salt_index])
            self.scores.append(s)
